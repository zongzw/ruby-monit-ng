...
class NeHwStat < Agent
  ...
  # ibm.allenvs.nessus.hwstat.[cpu_usage/mem_usage/disk_usage]
  def migrate
    metrics_list = []
    ...
    [:cpu_usage, :mem_usage].each do |item|
      info = {
          :sn => sysname + "-" + timestamp,
          :target => targetprefix + ".hwstat.#{item.to_s}",
          :instance => sysname,
          :status => @result[item],
          :details => "#{item.to_s}",
    ...
    @result[:disk_usage].each_key { |key|
      info = {
          :sn => timestamp,
          :target => targetprefix + ".hwstat.disk_usage",
          :instance => key,
